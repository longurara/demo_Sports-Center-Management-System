      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} xl={8}>
              ...
              <div style={{ marginTop: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 12 }}>
                  {[
                    ...
                    { l: 'Chuyên cần', v: <span><b>{profile.rate}%</b> <span style={{ color: '#9a968c' }}>/ {profile.att.length} buổi</span></span> },
                  ].map((x) => <div key={x.l} style={{ background: '#f7f5f0', borderRadius: 10, padding: '8px 10px' }}><div style={{ fontSize: 11, color: '#9a968c' }}>{x.l}</div><div style={{ fontSize: 13, fontWeight: 500 }}>{x.v}</div></div>)}
                </div>
                <div style={{ marginTop: 10, fontSize: 12, color: '#7a776f' }}>Lớp: {profile.cls.join(', ') || '—'}</div>
                {profile.bm.length > 1 && <div style={{ marginTop: 12 }}><BodyMetricsChart memberId={u.id} height={120} /></div>}
                {profile.results.length > 0 && (
                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 11, color: '#9a968c', marginBottom: 4 }}>KẾT QUẢ GẦN ĐÂY</div>
                    {profile.results.slice(0, 3).map((r) => <div key={r.id} style={{ fontSize: 12.5, display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px dashed #ece8df' }}><span>{r.metrics}</span><span style={{ color: '#9a968c' }}>{dayjs(r.s.date).format('DD/MM')}</span></div>)}
                  </div>
                )}
              </div>
            )}
          ...
          <Card title="2. Tham số lộ trình" size="small" style={{ marginTop: 16 }}>
            <div style={{ fontSize: 12, color: '#7a776f', marginBottom: 6 }}>Bộ môn trọng tâm (theo lớp học viên đang theo)</div>
            ...
            <div style={{ fontSize: 12, color: '#7a776f', marginBottom: 6 }}>Trọng tâm (AI tự suy từ mục tiêu, có thể chỉnh)</div>
            <Space wrap size={6}>
              {(Object.keys(FOCUS_LABEL) as Focus[]).map((f) => {
                const on = input.focus.includes(f);
                return <Tag key={f} onClick={() => setInput({ ...input, focus: on ? input.focus.filter((x) => x !== f) : [...input.focus, f] })} style={{ cursor: 'pointer', padding: '4px 12px', border: `1px solid ${on ? '#0f4d34' : '#e2ddd2'}`, background: on ? '#0f4d34' : '#fff', color: on ? '#fff' : '#3d3b35', fontWeight: 500 }}>{on && <CheckCircleFilled style={{ marginRight: 4 }} />}{FOCUS_LABEL[f]}</Tag>;
              })}
            </Space>
            <Divider style={{ margin: '14px 0' }} />
            <Row gutter={12}>
              <Col span={12}><div style={{ fontSize: 12, color: '#7a776f' }}>Số tuần</div><Segmented block value={input.weeks} onChange={(v) => setInput({ ...input, weeks: v as number })} options={[2, 4, 6, 8]} /></Col>
              <Col span={12}><div style={{ fontSize: 12, color: '#7a776f' }}>Buổi / tuần</div><Segmented block value={input.sessionsPerWeek} onChange={(v) => setInput({ ...input, sessionsPerWeek: v as number })} options={[2, 3, 4, 5]} /></Col>
            </Row>
            <div style={{ marginTop: 12, fontSize: 12, color: '#7a776f' }}>Thời lượng mỗi buổi: <b style={{ color: '#14130f' }}>{input.minutes} phút</b></div>
            <Slider min={30} max={90} step={15} value={input.minutes} onChange={(v) => setInput({ ...input, minutes: v })} marks={{ 30: '30', 60: '60', 90: '90' }} />
            <div style={{ fontSize: 12, color: '#7a776f', marginBottom: 4 }}>Thiết bị sẵn có</div>
            ...
            <div style={{ fontSize: 12, color: '#7a776f', margin: '12px 0 4px' }}>Ghi chú thêm cho AI (tùy chọn)</div>
        ...
        <Col xs={24} xl={16}>
          ...
          {u && step >= 0 && !plan && (
            <Card>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg,#0f4d34,#2f7d55)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}><RobotOutlined /></div>
                <div><b>AI đang phân tích hồ sơ của {u.fullName}</b><div style={{ fontSize: 12, color: '#7a776f' }}>Sử dụng {profile?.att.length ?? 0} bản ghi điểm danh · {profile?.results.length ?? 0} kết quả buổi tập · {profile?.bm.length ?? 0} lần đo chỉ số</div></div>
              </div>
          ...
          {u && !plan && step < 0 && (
            <Card>
              <div style={{ textAlign: 'center', padding: '32px 0' }}>
                <div style={{ width: 64, height: 64, borderRadius: 18, background: 'linear-gradient(135deg,#0f4d34,#2f7d55)', color: '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, boxShadow: '0 12px 30px rgba(124,58,237,.3)' }}><RobotOutlined /></div>
                <h3 style={{ marginBottom: 4 }}>Sẵn sàng xây lộ trình cho {u.fullName.split(' ').pop()}</h3>
                <div style={{ color: '#7a776f', maxWidth: 460, margin: '0 auto' }}>AI sẽ dùng mục tiêu, trình độ, ghi chú sức khỏe, chuyên cần, chỉ số cơ thể và kết quả buổi tập để đề xuất bài tập, số hiệp, thời gian nghỉ và tiến trình cường độ theo tuần.</div>
                <div style={{ display: 'flex', gap: 24, justifyContent: 'center', marginTop: 20, flexWrap: 'wrap' }}>
                  {[['Cá nhân hóa', 'theo 6 nguồn dữ liệu'], ['An toàn', 'lọc bài theo ghi chú sức khỏe'], ['Chỉnh sửa được', 'HLV duyệt trước khi gửi']].map(([a, b]) => <div key={a} style={{ textAlign: 'center' }}><div style={{ fontWeight: 600 }}>{a}</div><div style={{ fontSize: 12, color: '#9a968c' }}>{b}</div></div>)}
                </div>
              </div>
            </Card>
          )}
          ...
          {u && plan && (
            <div className="sc-fade">
              {/* Header kết quả */}
              <Card style={{ background: 'linear-gradient(135deg,#14130f 0%,#0f4d34 100%)', color: '#fff', border: 'none' }} styles={{ body: { padding: 22 } }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ textAlign: 'center', minWidth: 120 }}>
                    <Progress type="dashboard" percent={plan.confidence} size={96} strokeColor={{ '0%': '#e07a4f', '100%': '#22c55e' }} railColor="rgba(255,255,255,.15)" format={(p) => <span style={{ color: '#fff', fontWeight: 700 }}>{p}%</span>} />
              ...
              <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
                <Col xs={24} md={14}>
                  <Card size="small" title={<span><BulbOutlined style={{ color: '#f59e0b' }} /> Vì sao AI đề xuất như vậy</span>} style={{ height: '100%' }}>
                    <ul style={{ paddingLeft: 18, margin: 0, fontSize: 13, lineHeight: 1.7, color: '#3d3b35' }}>{plan.reasons.map((r) => <li key={r}>{r}</li>)}</ul>
                  </Card>
                </Col>
                <Col xs={24} md={10}>
              ...
              {/* Lộ trình theo tuần */}
              <Card style={{ marginTop: 16 }} title="Lộ trình chi tiết" extra={<span style={{ fontSize: 12, color: '#9a968c' }}>Bấm vào số hiệp / rep để chỉnh · xóa bài không phù hợp</span>}>
                  ...
                  children: (
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                        <b>{w.theme}</b>
                        <Progress percent={w.intensity} size="small" style={{ flex: 1, maxWidth: 260, margin: 0 }} strokeColor={w.intensity >= 90 ? '#dc2626' : w.intensity >= 75 ? '#c94a1e' : '#16a34a'} format={(p) => `cường độ ${p}%`} />
                      </div>
                      <Collapse defaultActiveKey={['0']} items={w.sessions.map((s, si) => ({
                        key: String(si),
                        label: <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}><span><Tag color="blue" style={{ marginRight: 8 }}>{s.day}</Tag><b>{s.title}</b></span><span style={{ fontSize: 12, color: '#9a968c' }}>{s.exercises.length} bài · {s.minutes} phút</span></div>,
                        children: (
                          <div>
                            <div style={{ fontSize: 12.5, color: '#3d3b35', marginBottom: 8 }}><b>Khởi động:</b> {s.warmup}</div>
                            <Table size="small" pagination={false} rowKey={(r) => r.name} dataSource={s.exercises} columns={[
                              { title: '#', render: (_, __, i) => i + 1, width: 40 },
                              { title: 'Bài tập', render: (_, e) => <><b>{e.name}</b>{e.note && <div style={{ fontSize: 11.5, color: '#9a968c' }}>{e.note}</div>}</> },
                              { title: 'Nhóm', dataIndex: 'group', render: (v) => <Tag style={{ background: '#f3f1ec', color: '#3d3b35' }}>{v}</Tag> },
                            ...
                            <div style={{ fontSize: 12.5, color: '#3d3b35', marginTop: 8 }}><b>Thả lỏng:</b> {s.cooldown}</div>
                          </div>
                        ),
                      }))} />
                    </div>
              ...
              <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
                <Col xs={24} md={14}>
                  <Card size="small" title="Dinh dưỡng & phục hồi">
                    <ul style={{ paddingLeft: 18, margin: 0, fontSize: 13, lineHeight: 1.7, color: '#3d3b35' }}>{plan.nutrition.map((n) => <li key={n}>{n}</li>)}</ul>
                  </Card>
                </Col>
                <Col xs={24} md={10}>
