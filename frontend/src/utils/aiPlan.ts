...
interface Lib { name: string; group: string; focus: Focus[]; equipment: PlanInput['equipment'][]; level: number; note?: string }
const LIB: Lib[] = [
...
export function generatePlan(u: User, data: AppData, input: PlanInput): AiPlan {
  ...
  const pool = LIB.filter((e) => e.equipment.includes(input.equipment) && e.level <= lvl + (attRate > 85 ? 1 : 0))
  ...
  const score = (e: Lib) => e.focus.filter((f) => focus.includes(f)).length * 2 + (e.level === lvl ? 1 : 0) + rnd() * 0.8;
  ...
  const sets = (e: Lib, week: number) => (/Cardio|Mobility/.test(e.group) ? 1 : Math.min(5, (lvl >= 2 ? 4 : 3) + (week >= 3 ? 1 : 0)));
  const rest = (e: Lib) => (/Cardio/.test(e.group) ? '—' : focus.includes('STRENGTH') ? '120s' : losing ? '45s' : '60–90s');
  ...
  const weeks: Week[] = Array.from({ length: input.weeks }).map((_, wi) => {
    ...
    const sessions: Session[] = Array.from({ length: input.sessionsPerWeek }).map((_, si) => {
      ...
      const cardioFirst = losing ? 'Cardio nhẹ 10 phút (máy chạy / xe đạp) + khởi động khớp' : 'Khởi động động 8 phút: jumping jack, arm circle, leg swing';
  ...
  return {
    title: `Lộ trình ${input.weeks} tuần · ${focus.map((f) => FOCUS_LABEL[f]).join(' & ')}`,
    ...
    tags: [...focus.map((f) => FOCUS_LABEL[f]), u.level === 'ADVANCED' ? 'Nâng cao' : u.level === 'INTERMEDIATE' ? 'Trung bình' : 'Người mới', `${input.sessionsPerWeek} buổi/tuần`],
