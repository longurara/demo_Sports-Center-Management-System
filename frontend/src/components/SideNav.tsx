...
import { LeftOutlined, LogoutOutlined, RightOutlined, ThunderboltFilled } from '@ant-design/icons';
...
export default function SideNav({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <div className={`sc-nav${collapsed ? ' collapsed' : ''}`}>
      {/* Brand */}
      <div className="sc-nav-brand">
        ...
        <Tooltip title={collapsed ? 'Mở rộng' : 'Thu gọn'} placement="right">
          <button className="sc-nav-toggle" onClick={onToggle}>{collapsed ? <RightOutlined /> : <LeftOutlined />}</button>
        </Tooltip>
      </div>
        ...
        {sections.map((sec, i) => (
          <div key={i} className="sc-nav-section">
            ...
            {sec.items.map((it) => {
              ...
              const node = (
                <Link key={it.key} to={it.key} className={`sc-nav-item${active ? ' active' : ''}`}>
      ...
      {/* User */}
      <div className="sc-nav-user" onClick={() => navigate(`/${role.toLowerCase()}/profile`)}>
