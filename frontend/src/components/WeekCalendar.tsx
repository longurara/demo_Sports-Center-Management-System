...
interface Props { weekStart: dayjs.Dayjs; events: CalEvent[]; hourFrom?: number; hourTo?: number; hourHeight?: number; dayCount?: number }

const toMin = (t: string) => Number(t.slice(0, 2)) * 60 + Number(t.slice(3, 5));

/** Lịch tuần dạng trục thời gian (giống Google Calendar): 7 cột ngày × khung giờ, sự kiện đặt theo giờ thực. */
export default function WeekCalendar({ weekStart, events, hourFrom = 6, hourTo = 22, hourHeight = 46, dayCount = 7 }: Props) {
